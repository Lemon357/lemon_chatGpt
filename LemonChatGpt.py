...
with gr.Blocks() as demo:
    ...
    with gr.Accordion(
        ...
        with gr.Column():
            ...
            with gr.Row():
                with gr.Column(scale=6):
                    uploadDropdown = gr.Dropdown(label="从列表中加载对话", choices=get_history_names(plain=True),
                                                 multiselect=False)
                with gr.Column(scale=1):
    ...
    retryBtn.click(predict, [txt, top_p, temperature, keyTxt, chatbot, history,
                             systemPromptTxt, TRUE_CONSTANT], [chatbot, history], show_progress=True)
    ...
    reduceTokenBtn.click(predict, [txt, top_p, temperature, keyTxt, chatbot, history,
                                   systemPromptTxt, FALSE_CONSTANT, TRUE_CONSTANT], [chatbot, history],
                         show_progress=True)
    ...
    uploadBtn.click(load_chat_history, [uploadDropdown], [saveFileName, systemPromptTxt, history, chatbot],
                    show_progress=True)
